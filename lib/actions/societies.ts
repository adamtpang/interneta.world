...
export async function getSocieties(): Promise<SocietyDatabase[]> {
  ...
  try {
    ...
    console.warn('Failed to fetch societies from Supabase, using static fallback:', error);

    const { societiesFull } = await import('@/lib/data/societies-full');
    return societiesFull;
  }
}
