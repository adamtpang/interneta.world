...
import { Users, MapPin, ChevronDown, ChevronUp, Calendar, Tag, ArrowUpDown } from "lucide-react";
...
const SORT_OPTIONS: { key: SortKey; label: string; why: string }[] = [
  ...
  { key: 'qol', label: 'Quality of Life', why: 'Sustainability & wellbeing' },
  { key: 'belonging', label: 'Belonging', why: 'Emotional cohesion & identity' },
  { key: 'purpose', label: 'Purpose', why: 'Clarity of shared mission' },
  { key: 'economic', label: 'Economic', why: 'Fair livelihood & wealth-building' },
  { key: 'events', label: 'Upcoming Events', why: 'Live activity right now' },
  { key: 'tier', label: 'Tier', why: 'Editorial ranking' },
  { key: 'name', label: 'Name (A–Z)', why: 'Alphabetical' },
];
...
// Combines: upcoming events (real-world activity), age (proven over time),
// tier (editorial curation: tier 1 = most established).
function getConfidence(
  s: { tier: number; founded?: string },
  upcomingEvents: number,
  currentYear: number
): number {
  const ageYears = s.founded ? Math.max(0, currentYear - parseInt(s.founded, 10)) : 0;
  const validAge = Number.isFinite(ageYears) ? ageYears : 0;
  const tierWeight = Math.max(0, 4 - s.tier) * 5;
  return upcomingEvents * 10 + validAge + tierWeight;
}

// Bayesian-weighted score: blends raw quality with a neutral prior, weighted by confidence.
// weighted = (confidence · quality + M · prior) / (confidence + M)
function getWeightedScore(
  ...
  return Math.round(
    (confidence * rawQuality + BAYES_M * NEUTRAL_PRIOR) / (confidence + BAYES_M)
  );
}
...
const getOverallScore = (s: {
  ...
  const vals = [s.scalability, s.autonomy, s.qol, s.belonging, s.purpose, s.economic];
  if (vals.some(v => v == null)) return null;
  const sum = vals.reduce((a, b) => (a as number) + (b as number), 0) as number;
  return Math.round(sum / vals.length);
};

const getScoreForSort = (s: SocietyDatabase, key: SortKey): number | null => {
  switch (key) {
    case 'overall': return getOverallScore(s);
    case 'scalability': return s.scalability ?? null;
    case 'autonomy': return s.autonomy ?? null;
    case 'qol': return s.qol ?? null;
    case 'belonging': return s.belonging ?? null;
    case 'purpose': return s.purpose ?? null;
    case 'economic': return s.economic ?? null;
    default: return null;
  }
};
  ...
  const filteredAndSortedSocieties = useMemo(() => [...societies]
    ...
    .sort((a, b) => {
      ...
      if (sortKey === 'tier') {
        ...
        return (getOverallScore(b) ?? -1) - (getOverallScore(a) ?? -1);
      }
      if (sortKey === 'events') {
      ...
      if (sortKey === 'weighted') {
        const confA = getConfidence(a, upcomingCountBySociety.get(a.name) ?? 0, currentYear);
        const confB = getConfidence(b, upcomingCountBySociety.get(b.name) ?? 0, currentYear);
        const wA = getWeightedScore(getOverallScore(a), confA);
        const wB = getWeightedScore(getOverallScore(b), confB);
        if (wA == null && wB == null) return a.tier - b.tier;
        if (wA == null) return 1;
        if (wB == null) return -1;
    ...
    [societies, deferredSearchTerm, selectedLocations, selectedTypes, upcomingCountBySociety, sortKey, currentYear]
  );

  const activeSort = SORT_OPTIONS.find(o => o.key === sortKey) ?? SORT_OPTIONS[0];
  const isScoreSort = sortKey !== 'events' && sortKey !== 'tier' && sortKey !== 'name';
              ...
              <div className="px-4 pb-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                {SORT_OPTIONS.map(opt => (
                  <button
                    ...
                    title={opt.why}
                    className={`text-left px-3 py-2 border-2 font-mono text-xs transition-colors ${
                      sortKey === opt.key
                  ...
                  >
                    ...
                    <div className={`text-[10px] mt-0.5 ${sortKey === opt.key ? 'opacity-90' : 'opacity-60'}`}>
                      {opt.why}
                    </div>
                  </button>
                ))}
                ...
                <Link href={`/societies/${societyNameToSlug(society.name)}`} className="block hover:opacity-80 transition-opacity">
                  <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2">
                        <SocietyBadges
                          ...
                          communityScore={(() => {
                            if (sortKey === 'weighted') {
                              const conf = getConfidence(society, upcomingCountBySociety.get(society.name) ?? 0, currentYear);
                              return getWeightedScore(getOverallScore(society), conf) ?? undefined;
                            }
                            if (isScoreSort) return getScoreForSort(society, sortKey) ?? undefined;
                            return getOverallScore(society) ?? undefined;
                          })()}
                        />
                      </div>
                    </div>
                    ...
                    {(() => {
                      ...
                      const overall = getOverallScore(society);
                      const confidence = getConfidence(society, upcomingCountBySociety.get(society.name) ?? 0, currentYear);
                      let activeScore: number | null = overall;
                      let activeLabel = 'Overall';
                      if (sortKey === 'weighted') {
                        activeScore = getWeightedScore(overall, confidence);
                        activeLabel = 'Weighted';
                      } else if (isScoreSort) {
                        activeScore = getScoreForSort(society, sortKey);
                        activeLabel = activeSort.label;
                      }
                      return (
                        <div className="hidden sm:flex flex-shrink-0 items-center gap-3">
                          <div className="text-right">
                            <div className="text-[10px] font-mono uppercase text-muted-foreground leading-tight">
                              {activeLabel}
                            </div>
                            <div className="text-2xl font-bold font-mono text-primary leading-none">
                              {activeScore ?? '—'}
                            </div>
                            <div className="text-[10px] font-mono text-muted-foreground leading-tight">/ 100</div>
                            {sortKey === 'weighted' && overall != null && (
                              <div className="text-[9px] font-mono text-muted-foreground leading-tight mt-0.5">
                                raw {overall} · conf {confidence}
                              </div>
                            )}
                          </div>
                          <MiniRadar
